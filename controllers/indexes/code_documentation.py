...
class CodeDocumentationVisitor(ast.NodeVisitor):
    ...
    def visit_FunctionDef(self, node: ast.FunctionDef, is_top_level: bool = False, parent_class: str = None):  # type: ignore
        ...
        for i, arg in enumerate(node.args.args):
            ...
            default_value = node.args.defaults[i - (len(node.args.args) - num_args_with_defaults)] if has_default else None

            argument = Argument(
                name=arg.arg,
                arg_type=astunparse.unparse(arg.annotation).strip() if arg.annotation else None,
